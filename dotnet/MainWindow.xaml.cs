...
namespace Testiamte
{
    public partial class MainWindow : System.Windows.Window, IComponentConnector
    {
        ...
        {
            MainWindow.VideoCapture = new VideoCapture(VideoCaptureConfig.CameraId, VideoCaptureAPIs.DSHOW);
            if (!MainWindow.VideoCapture.IsOpened())
                return;
            MainWindow.VideoCapture.FrameWidth = VideoCaptureConfig.Width;
            MainWindow.VideoCapture.FrameHeight = VideoCaptureConfig.Height;
            MainWindow.VideoCapture.AutoFocus = VideoCaptureConfig.AutoFocus;
        }
        ...
        private void StopCamera()
        {
            try
            {
                if (MainWindow.VideoCapture == null)
                    return;
                MainWindow.VideoCapture.Release();
            }
            catch (Exception ex)
            {
            }
        ...
        private string CaptureCamera()
        {
            if (MainWindow.VideoCapture != null)
            {
                if (!MainWindow.VideoCapture.IsDisposed)
                {
                    try
                    {
                        Mat image = new Mat();
                        ...
                        string fileName = Path.Combine(str, path2);
                        MainWindow.VideoCapture.Read(image);
                        if (image.Empty())
                            return string.Empty;
        ...
        private async void DispatcherTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                string filePath = this.CaptureScreen();
                string pathCamera = this.CaptureCamera();
